...
function CertificateProfile() {
    ...
    return (
      <div className="min-h-screen bg-soft-gray flex items-center justify-center">
        <div className="text-primary font-medium animate-pulse flex items-center gap-2">
          <Activity className="h-5 w-5 animate-spin" /> Loading verification details...
        </div>
      </div>
    );
  }
  ...
  if (error || !profile) {
    return (
      <div className="min-h-[70vh] bg-soft-gray flex flex-col items-center justify-center p-6">
        <div className="bg-white p-10 rounded-2xl border border-border shadow-sm max-w-md w-full text-center">
          <XCircle className="h-14 w-14 text-red-500 mx-auto mb-4" />
          <h2 className="text-2xl font-bold text-navy mb-2">Certificate Not Found</h2>
          <p className="text-muted-foreground mb-6">
            {error || "The requested certificate could not be found or may be invalid."}
          </p>
          <Link
            to="/directory"
            ...
            <div className="rounded-xl border border-border p-6 space-y-5">
              ...
              {profile.qrCode && (
                <div className="pt-2 flex flex-col items-center gap-2">
                  ...
                  <img src={profile.qrCode} alt="Verification QR Code" className="h-28 w-28 object-contain rounded border border-border" />
                </div>
              )}
            </div>
          </div>
